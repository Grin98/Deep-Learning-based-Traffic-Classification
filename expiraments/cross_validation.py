...
from misc.utils import create_dir, _create_pre_trained_model
...
class CrossValidation(Experiment):
    ...
    def run(self, data_dir=None, out_dir=None, bs_train=128, bs_test=256, epochs=40, early_stopping=3,
        ...
        if load_checkpoint:
            start_i, k, f1, acc, loss = self.load_cv(cv_checkpoint, default_k=k)
        ...
        for i in (start_i, k):
            ...
            model = self.create_model(input_shape, num_classes, filters_per_layer, layers_per_block, hidden_dims,
                                      drop_every)
    ...
    @staticmethod
    def load_cv(cv_checkpoint: str, default_k: int):
